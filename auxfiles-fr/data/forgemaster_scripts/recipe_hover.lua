...
local any_phatom = {"phantom_goul","phantom","phantom_mare","phantom_wraith",displayName="[EXPURGÉ]"}
local any_AC_1 = {
    "GB_AC_1",
    "GB_AC_1_CASELESS",
    "GB_AC_1_HV",
    ...
    "GB_AC_1_FLECHETTE",
    "GB_AC_1_RAD",
    displayName ="N’importe quel AC1"
}
local any_AC_2 = {
    "GB_AC_2",
    "GB_AC_2_CASELESS",
    ...
    "GB_AC_2_FLECHETTE",
    "GB_AC_2_RAD",
    displayName ="N’importe quel AC2"
}
local ANY_ASSAULT_DRONE = {"ASSAULT_PIERCE","ASSAULT_PARTICLE",displayName = "Drone d’Assaut"}
local ANY_COMBAT_2 = {"COMBAT_2_LASER","COMBAT_2_BEAM",displayName = "Drone de Bataille II"}
local ANY_COMBAT_1 = {"COMBAT_1_LASER","COMBAT_1_BEAM",displayName = "Drone de Bataille I"}
local ANY_ENERGY_AMP = {"ENERGY_1_AMP2","ENERGY_1_AMP3","ENERGY_1_AMP4",displayName = "Drone Énergétique Amplifié"}
local ANY_FOCUS_AMP = {"FOCUS_1_AMP2","FOCUS_1_AMP3","FOCUS_1_AMP4",displayName = "Drone Aiguillon Amplifié"}
local ANY_COMBAT_CONSERVATIVE = {"COMBAT_CONSERVATIVE_BEAM","COMBAT_CONSERVATIVE_LASER",displayName = "Drone Conservateur"}
local ANY_BATTLE_DRONE = {"BATTLETOG_DEFEND","BATTLETOG_BOARDER",displayName = "Drone de combat"}
local ANY_PEST_DRONE = {"PEST_RAD","PEST_DEBUFF",displayName = "Drone Ravageur"}
local ANY_RECON_DRONE = {"RECON_BOARDER","RECON_DEFENSE",displayName = "Drone de reconnaissance" }
...
mods.Forgemaster.recipes = {
    ...
    FM_BEAM_GUILLOTINE_CHAIN = {
        { 
            costs = {110,2,5},
            {"NANOBOT_DEFENSE_SYSTEM","LOCKED_NANOBOT_DEFENSE_SYSTEM", displayName = "[Expurgé]"},
			{"BEAM_GUILLOTINE"},
		}
    },
    ...
    FM_SHOTGUN_AETHER = {
        {  
            costs = 60,
			{"SHOTGUN_4"},
            {"BLUELIST_AETHER","ANCIENT_LASER",displayName ="une technologie oubliée"},
    ...
    FM_SHOTGUN_CURSED = {
        {  
            costs = 30,
			{"SHOTGUN_2"},
			{"lanius", displayName = "lanius et baie de clonage niv 3"},
    ...
    MAMMOTH_CANNON = {
        {
            costs = 60,
			{"MISSILES_V5_SINGLE","MISSILES_V5_MULTI", displayName = "N’importe quels Missiles V5"},
			any_AC_2,
            any_AC_2,
		}
    },
...
local function weapon_title_from_name(name)
    ...
    if string.len(ret) == 0 then return "[EXPURGÉ]" end
    return ret
end
-- taking in 
...
local function ingredient_list_to_string(ingredientList)
    ...
    local ret = table_to_list_string(ingredientList_truenames," ou ")
...
local forgestring = " Forger avec %s pour faire %s (%s)." 
local forgestring_alone = " Forger pour faire %s (%s)." 
    ...
    for recipe_index, recipe in ipairs(recipeList) do--iterating over the 1-2 recipes
        ...
        if valid_recipe and not invalid_recipe then
            if(#ingredient_strings>0) then
                table.insert(recipeStrings,forgestring:format(table_to_list_string(ingredient_strings, " et "),"%s",get_cost_string(recipe.costs or 0)))
            else
